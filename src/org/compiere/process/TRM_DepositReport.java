	...
	private void CreateLines(Timestamp beginperiod, Timestamp endperiod, MAttachmentEntry entry) throws WriteException
	{
		...
		try
		{
			...
			while(rs.next())
			{
		    	...
		    	number = new Number(col+2, index, rs.getBigDecimal("BeginningBalance") == null ? 0.0 : rs.getBigDecimal("BeginningBalance").doubleValue(), numcolstyle);
				sheet.addCell(number);
				
				number = new Number(col+3, index, rs.getBigDecimal("EndingBalance") == null ? 0.0 : rs.getBigDecimal("EndingBalance").doubleValue(), numcolstyle);
				sheet.addCell(number);
				...
				BigDecimal startBalance = rs.getBigDecimal("BeginningBalance") == null ? new BigDecimal(0) : rs.getBigDecimal("BeginningBalance");
				BigDecimal endBalance = rs.getBigDecimal("EndingBalance") == null ? new BigDecimal(0) : rs.getBigDecimal("EndingBalance");
				
				number = new Number(col+5, index, endBalance.subtract(startBalance).doubleValue() , numcolstyle);
				sheet.addCell(number);
        		...
        		number = new Number(col+12, index, rs.getBigDecimal("Premium") == null ? new BigDecimal(0).doubleValue() : rs.getBigDecimal("Premium").doubleValue(), numcolstyle);
				sheet.addCell(number);
